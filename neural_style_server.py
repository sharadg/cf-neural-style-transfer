...
class NeuralStyleRPCServer(object):
    ...
    def on_request(self, ch, method, properties, body):
        ...
        num_iterations = int(req["num_iterations"]) if req["num_iterations"] else 20

        content_image = plt.imread(io.BytesIO(raw_img_bytes), format=image_format)
        ...
        print(" [x] Sending Response: " + req["filename"])

        try:
            self.publish(response, properties.content_type, properties.correlation_id, properties.reply_to, method)

        except pika.exceptions.ConnectionClosed:
            # In case of sending a reply message, if the channel is closed then the message will be redelivered
            print("Exception while sending response, reconnecting...")
            self.connect()
            self.publish(response, properties.content_type, properties.correlation_id, properties.reply_to, method)
...
def process_message(content_image, style_image, num_iterations, stylized_image_arr, format):
    stylized_image_arr.append(neural_style_transfer.model_nn(content_image, style_image, num_iterations, format))


def resize_image(image_bytes, width=800, height=600, format="jpeg"):
    # convert nympy array image to PIL.Image
    image_array = plt.imread(io.BytesIO(image_bytes), format=format).astype(np.float)
